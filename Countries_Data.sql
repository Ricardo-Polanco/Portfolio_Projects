...
/* We get the country with the highest population */
SELECT name, MAX(population) AS Highest_Population FROM countries 
GROUP BY name ORDER BY Highest_Population DESC;

/* We get the country with the lowest population */
SELECT name, MIN(population) AS Lowest_Population FROM countries 
GROUP BY name ORDER BY Lowest_Population;

/* We get the total population of the world */
SELECT SUM(CAST(population AS bigint)) AS Total_World_Population FROM countries;

/* We get the average population of the world and then query the countries that have a population higher than the world average */
SELECT AVG(CAST(population AS bigint)) AS Average_Population FROM countries; 
SELECT name,population FROM countries WHERE population > 38774455;

/* We get the average fertility of the world and then query the countries that have a fertility rate higher 
than the world average and net migrants greater than zero. The data is sorted by highest net migrants to lowest. 
The connection that could be made here is between having positive net migrants and how that contributes to having a higher fertility rate */
SELECT AVG(fertility_rate) AS Average_Fertility_Rate FROM countries;
SELECT name, fertility_rate, net_migrants FROM countries 
GROUP BY name, fertility_rate, net_migrants 
HAVING fertility_rate >= 2.7 AND net_migrants > 0 ORDER BY net_migrants DESC;

/* We get the number of countries that have a higher than averge fertility rate */ 
Select count(name) AS Number_of_countries_with_a_higher_than_average_fertility_rate FROM countries 
WHERE fertility_rate >= 2.7;

/* We get the number of countries that have a higher than averge fertility rate and have positive net migrants */
Select count(name) AS Number_of_countries_with_a_higher_than_average_fertility_rate_and_positive_net_migrants FROM countries 
WHERE fertility_rate >= 2.7 AND net_migrants > 0; 

/* We see the average median age of the population of the world and the number of countries in this dataset */
SELECT AVG(median_age) AS Average_Median_Age FROM countries;
SELECT COUNT(name) AS Number_of_countries FROM countries;

/* Created a category that classifies the number of countries by median age. 
The countries_category was created by me and denotes what category the countries are in */
SELECT COUNT(name) AS Number_of_countries, 
    CASE
        WHEN median_age > 45 THEN 'High Longevity'
        WHEN median_age > 30 THEN 'Workforce'
        WHEN median_age > 20 THEN 'Normal'
        ELSE 'Very Fertile'
    END AS Countries_category
FROM countries
GROUP BY 
	CASE
        WHEN median_age > 45 THEN 'High Longevity'
...
/* Created a category that classifies the number of countries by the change in one year percentage. 
The countries_category was created by me and denotes what category the countries are in */
SELECT COUNT(name) AS Number_of_countries, 
    CASE
        WHEN percent_one_year_change > 3 THEN 'Excellent'
        WHEN percent_one_year_change > 2 THEN 'Great'
        WHEN percent_one_year_change > 1 THEN 'Good'
        WHEN percent_one_year_change > 0 THEN 'Okay'
        ELSE 'Terrible'
    END AS Countries_category
FROM countries
GROUP BY 
